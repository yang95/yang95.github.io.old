...
class Common_util_pub
{
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".WxPaypubconfig::KEY;
	...
	{		
        //初始化curl        
       	$ch = curl_init();
		//设置超时
		curl_setopt($ch, WxPaypubconfig::CURLOP_TIMEOUT, $second);
        //这里设置代理，如果有的话
	...
	function postXmlSSLCurl($xml,$url,$second=30)
	{
		...
		curl_setopt($ch,CURLOPT_SSLCERT, WxPaypubconfig::SSLCERT_PATH);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, WxPaypubconfig::SSLKEY_PATH);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub
{
	...
	function createXml()
	{
	   	$this->parameters["appid"] = WxPaypubconfig::APPID;//公众账号ID
	   	$this->parameters["mch_id"] = WxPaypubconfig::MCHID;//商户号
...
class UnifiedOrder_pub extends Wxpay_client_pub
{
	function __construct()
	{
		...
		//设置curl超时时间
		$this->curl_timeout = WxPaypubconfig::CURL_TIMEOUT;
	}
...
class NativeCall_pub extends Wxpay_server_pub
{
	...
	function createXml()
	{
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = WxPaypubconfig::APPID;//公众账号ID
		   	$this->returnParameters["mch_id"] = WxPaypubconfig::MCHID;//商户号
...
class JsApi_pub extends Common_util_pub
{
	...
	{
		$urlObj["appid"] = WxPaypubconfig::APPID;
	...
	{
		$urlObj["appid"] = WxPaypubconfig::APPID;
		$urlObj["secret"] = WxPaypubconfig::APPSECRET;
	...
	function getOpenid()
	{
        ...
        //初始化curl
       	$ch = curl_init();
		//设置超时
		curl_setopt($ch, WxPaypubconfig::CURLOP_TIMEOUT, $this->curl_timeout);
		curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,FALSE);
        curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,FALSE);
		curl_setopt($ch, CURLOPT_HEADER, FALSE);
	...
	{
		$jsApiObj["appId"] = WxPaypubconfig::APPID;
		$timeStamp = time();
