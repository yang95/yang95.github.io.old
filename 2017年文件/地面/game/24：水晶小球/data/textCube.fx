...
vertexOutput VS_TransformAndTexture(vertexInput IN) 
{
    ...
    OUT.texCoordDiffuse = float4(IN.texCoordDiffuse,1,1);    
...
sampler BallSampler = sampler_state 
{
    texture = <ballTexture>;
    AddressU  = CLAMP;        
    AddressV  = CLAMP;
...
float4 PS_Textured( vertexOutput IN): COLOR
{
  ...
  float4 diffuseTexture = tex2D( TextureSampler,IN.MapUV.xy);  //*0.7f + tex2D( BallSampler,IN.texCoordDiffuse.xy)*0.3f;
