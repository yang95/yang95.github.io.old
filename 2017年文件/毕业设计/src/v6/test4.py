...
def Init(cap):
    ...
    while(1):
        ...
        ret,edge = cv2.threshold(gray,200,255,cv2.THRESH_BINARY)#threshold

         
        if (img_pre == None):
            img_pre = edge.copy()#previous image

        tmp=edge-img_pre #the difference
        
        img_pre = edge.copy()#update previous image

        
        #edge = cv2.blur(tmp,(5,5))#noise removal
        #edge = cv2.GaussianBlur(tmp,(105,105),20)   
        edge = cv2.medianBlur(tmp.copy(),15)
